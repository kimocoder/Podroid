...
class ZrleDecoder {
    ...
    companion object {
        ...
        // 16KB buffers minimize JNI transitions between JVM and native Inflater.
        private const val BUFFER_SIZE = 16 * 1024
    }

    // Scratch buffer for compressed input read from the socket.
    private val inputScratch = ByteArray(BUFFER_SIZE)

    // Decompressed output buffer; re-used across inflate calls.
    private val zOutputBuf = ByteArray(BUFFER_SIZE)
    private var zPos = 0
            ...
            subenc == 128 -> {
                ...
                while (filled < total) {
                    ...
                    while (remainingRun > 0) {
                        val count = minOf(remainingRun, tw - currCol)
                        val base = (ty + currRow) * stride + (tx + currCol)
                        if (count == 1) target[base] = color
                        else Arrays.fill(target, base, base + count, color)
    ...
    private fun fill() {
        while (zAvail == 0) {
            ...
            val n = inflater.inflate(zOutputBuf)
            if (n > 0) {
                zPos = 0
                zAvail = n
    ...
    private fun readByte(): Int {
        if (zAvail == 0) fill()
        zAvail--
        return zOutputBuf[zPos++].toInt() and 0xFF
    }

    /** Read 3-byte CPIXEL (B, G, R) and return as ARGB int. */
    private fun readCpixel(): Int {
        // Optimistically check for 3 bytes to avoid multiple fill() checks.
        if (zAvail < 3) {
            val b = readByte()
            val g = readByte()
            val r = readByte()
        ...
        val b = zOutputBuf[zPos++].toInt() and 0xFF
        val g = zOutputBuf[zPos++].toInt() and 0xFF
        val r = zOutputBuf[zPos++].toInt() and 0xFF
        zAvail -= 3
        return (0xFF shl 24) or (r shl 16) or (g shl 8) or b
    }
