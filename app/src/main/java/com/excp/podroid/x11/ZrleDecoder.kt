...
class ZrleDecoder {
    ...
    fun decode(din: DataInputStream, x: Int, y: Int, w: Int, h: Int, target: IntArray, stride: Int) {
        ...
        // Reset decompression buffer state for the new rect.
        zPos = 0
        zAvail = 0
        remaining = compLen
        inputStream = din
            ...
            subenc == 128 -> {
                ...
                while (filled < total) {
                    ...
                    while (remainingRun > 0) {
                        val canDo = minOf(remainingRun, tw - currCol)
                        val startIdx = (ty + currRow) * stride + (tx + currCol)
                        java.util.Arrays.fill(target, startIdx, startIdx + canDo, color)
    ...
    private fun readCpixel(): Int {
        // Fast path: if at least 3 bytes are available, read directly from buffer.
        // This avoids 3 calls to readByte() and associated fill() checks.
        if (zAvail >= 3) {
