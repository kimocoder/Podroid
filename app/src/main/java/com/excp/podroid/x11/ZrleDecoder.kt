...
class ZrleDecoder {
    ...
    // 16KB to minimize JNI transition overhead between JVM and native zlib.
    ...
    private fun decodeTile(tx: Int, ty: Int, tw: Int, th: Int, target: IntArray, stride: Int) {
        ...
        when {
            ...
            subenc == 128 -> {
                ...
                while (filled < total) {
                    val color = readCpixel()
                    var runLen = readRunLength()
                    if (filled + runLen > total) throw IOException("ZRLE: plain RLE run overruns tile ($filled+$runLen > $total)")
                    filled += runLen
                    while (runLen > 0) {
                        val len = minOf(runLen, tw - currCol)
                        java.util.Arrays.fill(target, (ty + currRow) * stride + (tx + currCol), (ty + currRow) * stride + (tx + currCol) + len, color)
                        currCol += len
                        if (currCol == tw) {
                            currCol = 0
                            currRow++
