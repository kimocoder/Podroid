    ...
    private fun decodeTile(zi: ZInput, tx: Int, ty: Int, tw: Int, th: Int, target: IntArray, stride: Int) {
        ...
        when {
            ...
            subenc == 1 -> {
                ...
                for (row in 0 until th) {
                    Arrays.fill(target, (ty + row) * stride + tx, (ty + row) * stride + tx + tw, color)
                }
            }
            subenc in 2..16 -> {
                // Packed palette.
            ...
            subenc == 128 -> {
                ...
                while (filled < total) {
                    val color = zi.readCpixel()
                    var remRun = zi.readRunLength()
                    if (filled + remRun > total) throw IOException("ZRLE: plain RLE run overruns tile ($filled+$remRun > $total)")
                    filled += remRun
                    while (remRun > 0) {
                        val runInRow = minOf(remRun, tw - currCol)
                        Arrays.fill(target, (ty + currRow) * stride + tx + currCol, (ty + currRow) * stride + tx + currCol + runInRow, color)
                        remRun -= runInRow
                        currCol += runInRow
                        if (currCol == tw) {
                            currCol = 0
            ...
            subenc in 130..255 -> {
                ...
                while (filled < total) {
                    ...
                    if (indexByte and 0x80 == 0) {
                        ...
                        var remRun = zi.readRunLength()
                        if (filled + remRun > total) throw IOException("ZRLE: palette RLE run overruns tile ($filled+$remRun > $total)")
                        filled += remRun
                        while (remRun > 0) {
                            val runInRow = minOf(remRun, tw - currCol)
                            Arrays.fill(target, (ty + currRow) * stride + tx + currCol, (ty + currRow) * stride + tx + currCol + runInRow, color)
